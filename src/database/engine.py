...
# Структура данных для DPAPI, определяется только под Windows
...
def _protect_key_dpapi(key: str) -> bytes:
    """Шифрует ключ с помощью Windows DPAPI или использует plain-text на других ОС."""
    if sys.platform != "win32":
        logger.warning(
            "DPAPI недоступно на данной платформе. Ключ БД будет сохранен в файле в открытом виде."
        )
    ...
    if not result:
        raise RuntimeError("Ошибка DPAPI: CryptProtectData не удалась")
...
def _unprotect_key_dpapi(protected: bytes) -> str:
    """Расшифровывает ключ с помощью Windows DPAPI или читает plain-text на других ОС."""
    if sys.platform != "win32":
    ...
    if not result:
        raise RuntimeError("Ошибка DPAPI: CryptUnprotectData не удалась")
...
def _get_or_create_key() -> str:
    ...
    if PROTECTED_KEY_PATH.exists():
        try:
            ...
            logger.error(
                f"Не удалось расшифровать ключ БД: {e}. Будет сгенерирован новый ключ."
            )
            pass

    logger.info("Генерация нового ключа шифрования БД.")
